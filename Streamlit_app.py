...
.btn-neon { width:100%; padding:14px; border-radius:14px; border:2px solid #00f6ff; background: rgba(0,255,255,0.07); color:#b9faff; font-size:18px; letter-spacing:1px; transition:0.25s; }
.btn-neon:hover { background:#00eaff; color:#00141a; transform:translateY(-2px); box-shadow:0 0 8px #00eaff,0 0 16px #00eaff,0 0 30px #00d8ff; }
...
# Session State Init
# -------------------------
for k in ["score","round","current_q","last_result","speed_start","speed_answer","game_mode"]:
    if k not in st.session_state:
        st.session_state[k] = 0 if k in ["score","round"] else None if k in ["current_q","speed_start"] else "" if k=="speed_answer" else "Easy"
...
def generate_question(mode):
    ...
    elif mode=="Speed": a,b,op=random.randint(1,15),random.randint(1,15),random.choice(["+","-","×"])
    else: a,b,op=random.randint(1,10),random.randint(1,10),random.choice(["+","-"])
