...
# -------------------------
    ...
    elapsed=time.time()-st.session_state.speed_start; remain=max(0,int(time_limit-elapsed))
...
# -------------------------
    ...
    if os.path.exists("leaderboard.csv"):
        ...
        for i,row in df.head(20).iterrows(): st.markdown(f"<div class='leader-row'>#{i+1} {row['name']} — {int(row['score'])} pts ({row.get('mode','')})</div>",unsafe_allow_html=True)
